...
plt_classes = [389, 390, 349]  # Alarm Clock, Siren, Doorbell
...
target_words = ["fire", "sunog", "pulis", "police", "warning",  "magnanakaw", "intruder", "tulong", "run", "takbo", "tumakbo",
                "emergency", "trouble", "evacuate", "danger",  "iwas", "ingat", "call", "alert", "alarm", "lindol", "saklolo", 
                "excuse", "atras", "abante", "alis", "tigil", "tumigil", "bilis", "layo","lumayo", "slippery", "stop"]
...
class App:
    def __init__(self, root):
        ...
        self.label = Label(root, textvariable=self.label_text, font=self.active_font, fg="black", bg="white", width=20, height=3)  # Adjust width and height for smaller GUI
    ...
    def flash_word(self, detected_words):
        if detected_words:
            ...
            self.label.config(fg="yellow" if word in target_words else "yellow", bg="red")
            self.root.update()
            #vibrate(3)
